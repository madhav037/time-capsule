...
function Main() {
  ...
  const months_comp = months.map((month, index) => {
    return (
      <option className="text-black" value={month} selected={index == dt.getMonth() ? true : false}>
        {month}
      </option>
    );
  });
  ...
  const dates_comp = dates.map((date) => {
    return (
      <option className="text-black" value={date} selected={dt.getDate() == date ? true : false}>
        {date}
      </option>
    );
  });
  ...
  const years_comp = years.map((year) => {
    return <option className="text-black" value={year}>{year}</option>;
  });

  return (
    <>
      <div className="bg-[#C8CFA0] h-100vh">
        <Menu />
        <div className="my-2 mx-5">
        <div className="text-5xl font-semibold font-mono">Write a letter to your future self</div>
        <div className="text-xl font-medium font-mono">Write. Pick a receiving date. Send. Verify. That’s it 😊</div>
        </div>
